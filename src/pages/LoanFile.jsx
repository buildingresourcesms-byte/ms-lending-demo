...
export function ComposeModal({ b, channel, connections, officer, onClose }) {
  const { logCommunication, go } = useApp()
  ...
  const send = (e) => {
    e.preventDefault()
    if (channel === 'email') {
      logCommunication(b.id, 'email', `Email sent — “${subject}”`, `Email sent via ${prov.name}`)
    } else if (channel === 'sms') {
      const preview = body.trim().length > 44 ? body.trim().slice(0, 44) + '…' : body.trim()
      logCommunication(b.id, 'sms', `Text sent — “${preview}”`, `Text sent via ${prov.name}`)
    } else {
      logCommunication(
        b.id,
        'call',

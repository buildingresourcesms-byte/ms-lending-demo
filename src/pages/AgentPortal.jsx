...
import { BrandMark, Btn, Select, Field, Badge, ProgressBar, Avatar, inputCls, cx } from '../ui.jsx'

const BLANK = { name: '', phone: '', loanType: 'Conventional', purpose: 'Purchase' }

/* Self-serve pre-approval letters, capped by the LO per buyer.
   The letter matches the OFFER price (not max approval), so the
   agent never over-shows the buyer's hand — and never has to call
   at 9pm on a Saturday for a re-issue. */
function LetterGenerator({ deals, agent, officer, toast }) {
  const eligible = deals.filter((b) => b.preApprovalMax && b.purpose === 'Purchase')
  ...
  return (
    <div className="rounded-3xl bg-white p-6 shadow-sm ring-1 ring-slate-200/70">
      ...
      <p className="mt-1 text-xs text-slate-500">
        Writing an offer tonight? Generate the letter at your offer price — {officer.name.split(' ')[0]} pre-set
        each buyer’s ceiling, so it’s always accurate.
      </p>
...
export default function AgentPortal({ initialId }) {
  const { borrowers, addBorrower, seat, currentOfficer, toast } = useApp()
    ...
    <div>
      ...
      <div className="mb-6 flex flex-wrap items-center justify-between gap-3 rounded-2xl bg-navy-950 px-4 py-3.5 text-white">
        <p className="flex items-center gap-2 text-sm text-navy-100">
          ...
          <span>
            <span className="font-semibold text-white">Agent portal preview</span> — what your realtor partners
            see. No login, no training, no income or credit data.
          </span>
        </p>
        <Select
          value={agent.id}
      ...
      <div className="mx-auto max-w-2xl space-y-4">
        ...
        <div className="rounded-3xl bg-white p-6 shadow-sm ring-1 ring-slate-200/70">
          ...
          {active.length === 0 ? (
            <p className="py-6 text-center text-sm text-slate-400">
              No buyers in motion right now — send one below and watch it appear.
            </p>
          ) : (
        ...
        {/* send a referral — the 30-second handoff */}
        <div className="rounded-3xl bg-white p-6 shadow-sm ring-1 ring-slate-200/70">
          <h2 className="text-sm font-semibold text-navy-900">Send {officer.name.split(' ')[0]} a buyer</h2>
          <p className="mt-1 text-xs text-slate-500">
            30 seconds. They get a call within the hour, and you watch every step above.
          </p>
          {sentName ? (
            <div className="mt-4 rounded-2xl bg-sage-50 p-4 text-center ring-1 ring-sage-100">
              ...
              <p className="mt-2 text-sm font-semibold text-sage-900">{sentName} is in {officer.name.split(' ')[0]}’s pipeline!</p>
              <p className="mt-0.5 text-xs text-sage-700">
                You’ll see them appear in “Your buyers, live” the moment there’s movement.
              </p>
        ...
        {/* instant pre-approval letter — the 9pm-Saturday-call killer */}
        <LetterGenerator deals={active} agent={agent} officer={officer} toast={toast} />
        ...
        <div className="overflow-hidden rounded-3xl bg-gradient-to-br from-navy-900 to-navy-700 p-6 text-white shadow-md">
          ...
          <div className="mt-3 flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-xs text-navy-200">{officer.role} · {officer.nmls}</p>
            </div>
          </div>
